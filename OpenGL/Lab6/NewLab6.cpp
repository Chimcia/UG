...
for(Model m : models)
   ...
   // glClear( GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT );
	...
	//glUseProgram(0);
glUniform3fv( glGetUniformLocation( id, "Camera_Position" ), 1, &Camera_Position[0] );

		// (b) oswietlenie
		glUniform4fv( glGetUniformLocation(id, "myLight.Ambient"), 1, &Light1.Ambient[0]);
		glUniform4fv( glGetUniformLocation(id, "myLight.Diffuse"), 1, &Light1.Diffuse[0]);
		glUniform4fv( glGetUniformLocation(id, "myLight.Specular"), 1, &Light1.Specular[0]);
		glUniform4fv( glGetUniformLocation(id, "myLight.Position"), 1, &Light1.Position[0]);
		glUniform1f( glGetUniformLocation(id, "myLight.Attenuation"), Light1.Attenuation);
		...
		{
			// (a) material
			glUniform4fv( glGetUniformLocation(id, "myMaterial.Ambient"), 1, &Material1.Ambient[0]);
			glUniform4fv( glGetUniformLocation(id, "myMaterial.Diffuse"), 1, &Material1.Diffuse[0]);
			glUniform4fv( glGetUniformLocation(id, "myMaterial.Specular"), 1, &Material1.Specular[0]);
			glUniform1f( glGetUniformLocation(id, "myMaterial.Shininess"), Material1.Shininess);

			// (b) macierz rzutowania
			glm::mat4 matModel = glm::mat4(1.0);
			matProjView = matProj * matView * matModel;
			glUniformMatrix4fv( glGetUniformLocation(id, "matProjView" ), 1, GL_FALSE, &(matProjView[0][0]) );
		...
		{
			// (a) material
			glUniform4fv( glGetUniformLocation(id, "myMaterial.Ambient"), 1, &Material2.Ambient[0]);
			glUniform4fv( glGetUniformLocation(id, "myMaterial.Diffuse"), 1, &Material2.Diffuse[0]);
			glUniform4fv( glGetUniformLocation(id, "myMaterial.Specular"), 1, &Material2.Specular[0]);
			glUniform1f( glGetUniformLocation(id, "myMaterial.Shininess"), Material2.Shininess);
...
glUniform1i(glGetUniformLocation(id, "tex0"), 0);
		glDrawArrays( GL_TRIANGLES, 0, OBJ_vertices.size() );
			...
			glAttachShader( id, LoadShader(GL_VERTEX_SHADER, "vertex.glsl"));
	glAttachShader( id, LoadShader(GL_FRAGMENT_SHADER, "fragment.glsl"));
